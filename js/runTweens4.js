function runTweens(){
	...
	var dur1 = 30.*duration * (t1 - t01)/(tf - t00);
        var dur2 = 13.*duration * (t2 - t1)/(tf - t00);
        var dur3 = 600.*duration * (tf - t2)/(tf - t00) *50.;
	var dur4 = dur3/3.;
...
///////////////////////////////tween00 - Zoom in from wide view to just over Mars
...
///////////////////////////////tween1 - Zoom in to close up on asteroid
...
//////////////////////tween3 - slow down, turn toward Earth, shrink asteroid		
	var initialTime3 = {t:t2};
	var finalTime3 = {t:tf};
	var r1 = {r:5.e4};
	var r2 = {r:5.e2};
	var finalRot3 = { x: -0.23486571657174551, y: 0.8965041754447987, z: 0.7012401627878595 };
	var finalPos3 = { x: 0.72689694326781, y: -0.30256053688871265, z: 0.5938783162030453 };
	var timeTween3 = new TWEEN.Tween(initialTime3).to(finalTime3, dur3).easing(TWEEN.Easing.Sinusoidal.Out)
		.onUpdate(function(object){
			params.Year = object.t;
			params.updateSolarSystem();
		});
	var posTween3 = new TWEEN.Tween(camera.position).to(finalPos3, dur3).easing(TWEEN.Easing.Sinusoidal.In)
                .onUpdate(function(object){
                        camera.position.x = object.x;
                        camera.position.y = object.y;
                        camera.position.z = object.z;
...
///////////////////video
	...
	var rTween3 = new TWEEN.Tween(r1).to(r2, dur3).easing(TWEEN.Easing.Sinusoidal.Out)
              .onUpdate(function(object){
                      params.AquariusRadFac = object.r;
                      clearAquarius();
                      drawAquarius();
              });
	var rotTween3 = new TWEEN.Tween(camera.rotation).to(finalRot3, dur3).easing(TWEEN.Easing.Exponential.Out)
                .onUpdate(function(object){
...
///////////////////////////////tween4 - dummy tween to extend capture on last frame
	var foo1 = {x:0};
        var foo2 = {x:1};
	var posTween4 = new TWEEN.Tween(foo1).to(foo2, dur4).easing(TWEEN.Easing.Linear.None)
                .onStart(function(){
                        console.log("tween4 start")
                })		
		.onComplete(function() {
			console.log("tween4 end:",params.Year);
///////////////////video
		//	params.stopVideo();
		//	params.saveVideo();
///////////////////video
	...
	posTween2.chain(posTween3, rTween3, timeTween3);
	posTween3.chain(posTween4);
	posTween00.start();
